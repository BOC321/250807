  ...
  try {
    ...
    try {
      ...
      try {
        ...
        // Try to send via Maileroo as a backup
        try {
          console.log('Attempting to send email via Maileroo as fallback');
          
          const mailerooResponse = await fetch(process.env.NEXT_PUBLIC_MAILEROO_API_URL, {
            method: 'POST',
            headers: {
          ...
          if (mailerooResponse.ok) {
            console.log('Email sent successfully via Maileroo (fallback)');
            emailSent = true;
          } else {
            console.error('Fallback email failed via Maileroo:', await mailerooResponse.text());
          }
        } catch (fallbackError) {
